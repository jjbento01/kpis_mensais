...
def do_main()->tuple:
    ...
    date_range_ex = range_needed_of_date(datetime.today())
    ...
    ic(date_range_ex[0].strftime("%Y-%m-%d"))
    ic(primeiro_ultimo_mes)
    ic(date_range_ex[-1].strftime("%Y-%m-%d"))
    ...
    bloco_principal_logins(ws, first_last_month, last_last_month, primeiro_ultimo_mes, ultimo, lista_unica, date_range_ex, dados, tags_logins, estilos)
    ...
    for coluna, iweek, iday in zip(range(7,13), (fifth_week, fourth_week, third_week, second_week, first_week), (fifth_day, fourth_day, third_day, second_day, first_day)): 
        ws.cell(row=2, column=len(date_range_ex)+coluna, value="W"+str(iweek)).style=totalinhadir
        ws.cell(row=3, column=len(date_range_ex)+coluna, value=(datetime(iday.year, iday.month, iday.day, 0,0,0) - datetime(1899, 12, 30)).days-1).style=normalshort
    ...
    for linha, item in zip([7, 8, 9, 10, 11, 12], lista):
        for ipos, (year, week) , sty in zip(range(7, 13), [(fifth_year, fifth_week), (fourth_year, fourth_week), (third_year, third_week), (second_year, second_week),(first_year, first_week)], [nmgrds, nmgrds, nmgrds, nmgrds, nmgrds]):
            ...
            ws.cell(row=linha, column=len(date_range_ex)+ipos, value=temp).style=sty
    ...
    lista_semanas = [(first_year, first_week), (second_year, second_week), (third_year, third_week), (fourth_year, fourth_week), (fifth_year, fifth_week)]
    ...
    for coluna, (year, week) in zip(
        ...
        ws.cell(row=6, column=len(date_range_ex)+coluna, value=temp).style=totalinha
        ws.cell(row=13, column=len(date_range_ex)+coluna, value=per_user).style=normalunder

    coluna: int+p = len(date_range_ex)+12

    total_first: int = 0
    total_second: int = 0
    sequencia_formatos = [normalgrayperc, normalgrayperc, normalgrayperc, normalgrayperc, normalgrayperc, normalgrayperc]
    for linha, item, color in zip(range(len(lista_unica)), lista_unica, sequencia_formatos):
    ...
    letra_no_fim = ws.cell(row=5, column=(len(date_range_ex)-5)).column_letter
    ...
    coluna+=1
    coluna_vazia_fim(ws,coluna, ultimo)
    row_actual = 14
    coluna = 2
    ...
    coluna +=3
    calculate_week_columns(dados, 'logins_week_tv', 'Qtd_Visits_Unique', ws, coluna, date_range_ex, fifth_year, fifth_week, fourth_year, fourth_week, third_year, third_week, second_year, second_week, first_year, first_week, ['Qtd_Visits', 'Qtd_Visits_Intentional'], 15, 17)
    coluna += 4
